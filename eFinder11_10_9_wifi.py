...
def readNexus(): # Nexus USB port set to LX200 protocol
    ...
    print ('Nexus RA:  ',ra.hstr(format='{1:02}:{2:02}:{3:02}'),'  Dec: ',dec.dstr(format='{0:+>1}{1:02}:{2:02}:{3:02}'))
...
def move():
    ...
    goto_target = Star(ra_hours=(float(goto_ra[0]),float(goto_ra[1]),float(goto_ra[2])) \
                       ,dec_degrees=(float(goto_dec[0]),float(goto_dec[1]),float(goto_dec[2])))
    goto_Pos=location.at(ts.now()).observe(goto_target)
    alt_g,az_g,d = goto_Pos.apparent().altaz()
    alt_s,az_s,d = solvedPos.apparent().altaz()
    delta_Az = (az_g.degrees - az_s.degrees)*60 # +ve move scope right
    delta_Alt = (alt_g.degrees - alt_s.degrees)*60 # +ve move scope up
    delta_Az_str = "{:.1f}".format(delta_Az)
    delta_Alt_str = "{:.1f}".format(delta_Alt)
    ra,dec,d = goto_Pos.radec(epoch=ts.now())
    display ('moving dAz dAlt',delta_Az_str+','+delta_Alt_str,'')
    print ('GoTo++  RA: ',ra.hstr(format='{1:02}:{2:02}:{3:02}'),'  Dec: ',dec.dstr(format='{0:+>1}{1:02}:{2:02}:{3:02}'))
